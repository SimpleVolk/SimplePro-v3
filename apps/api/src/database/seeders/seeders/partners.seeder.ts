 ...
 */
export async function seedPartners(PartnerModel: any, users: any[]): Promise<any[]> {
  const partners: any[] = [];
  ...
  for (let i = 0; i < partnerCount; i++) {
    ...
    if (commissionType === 'percentage') {
      commissionStructure.rate = faker.number.float({ min: 5, max: 15, fractionDigits: 2 });
    ...
    const totalLeadsReferred = isActive ? faker.number.int({ min: 10, max: 100 }) : 0;
    const totalLeadsConverted = isActive
      ? faker.number.int({ min: 0, max: Math.floor(totalLeadsReferred * 0.7) })
      : 0;
    const conversionRate = totalLeadsReferred > 0
    ...
    const partner = await PartnerModel.create({
      ...
      statistics: {
        totalLeadsReferred,
        totalLeadsConverted,
        totalRevenue: totalLeadsConverted * faker.number.int({ min: 800, max: 3000 }),
        totalCommissionsPaid: totalLeadsConverted * faker.number.int({ min: 50, max: 200 }),
        conversionRate,
      },
      settings: {
        autoNotifyOnLeadUpdate: true,
        preferredContactMethod: faker.helpers.arrayElement(['email', 'phone', 'portal']),
        customFields: {},
      },

...
export class MemoryHealthIndicator extends HealthIndicator {
  ...
  private readonly MEMORY_THRESHOLD = parseFloat(
    process.env.MEMORY_THRESHOLD || '0.9',
  ); // 90%
  private readonly HEAP_THRESHOLD = parseFloat(
    process.env.HEAP_THRESHOLD || '0.9',
  ); // 90%
  ...
  async isHealthy(key: string): Promise<HealthIndicatorResult> {
    try {
      ...
      const memoryHealthy =
        systemInfo.memory.percentage < this.MEMORY_THRESHOLD;
      const heapHealthy =
        systemInfo.memory.heap.percentage < this.HEAP_THRESHOLD;
      ...
      if (!isHealthy) {
        ...
        if (!memoryHealthy) {
          issues.push(
            `System memory usage: ${(systemInfo.memory.percentage * 100).toFixed(1)}%`,
          );
        }
        if (!heapHealthy) {
          issues.push(
            `Heap memory usage: ${(systemInfo.memory.heap.percentage * 100).toFixed(1)}%`,
          );
        }

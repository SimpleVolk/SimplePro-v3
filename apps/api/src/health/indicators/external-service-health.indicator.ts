...
export class ExternalServiceHealthIndicator extends HealthIndicator {
  ...
  async isHealthy(
    ...
    try {
      const response = await firstValueFrom(
        this.httpService.get(url, {
          ...
          catchError(error => {
            ...
            throw new Error(`HTTP request failed: ${JSON.stringify(errorInfo)}`);
          })
        )
      );
      ...
      const responseTime = Date.now() - startTime;
      const errorMessage = error instanceof Error ? error.message : 'Unknown external service error';
   ...
   */
  getConfiguredServices(): Array<{ key: string; url: string; timeout?: number }> {

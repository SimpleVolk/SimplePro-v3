...
export class ReferralsService {
  ...
  async create(
    ...
    const referral = new this.referralModel({
      ...
      customerId: createReferralDto.customerId
        ? new Types.ObjectId(createReferralDto.customerId)
        : undefined,
  ...
  async findAll(query: ReferralQueryDto): Promise<{
    ...
    if (pendingCommission) {
      filter.$and = [{ status: 'won' }, { 'commissionDetails.isPaid': false }];
    }
  ...
  async findById(referralId: string): Promise<ReferralDocument> {
    const referral = await this.referralModel
      ...
      .populate(
        'partnerId',
        'companyName contactName email commissionStructure',
      )
      .populate('customerId', 'firstName lastName email phone')
  ...
  async findByPartner(
    partnerId: string,
    query?: Partial<ReferralQueryDto>,
  ): Promise<ReferralDocument[]> {
  ...
  async updateStatus(
    ...
    const daysSinceReferral = Math.ceil(
      (now.getTime() - referralDate.getTime()) / (1000 * 60 * 60 * 24),
    );

    // Track days to contact (when first contacted)
    if (
      updateStatusDto.status === 'contacted' &&
      !referral.conversionData.daysToContact
    ) {
    ...
    if (
      updateStatusDto.status === 'quoted' &&
      !referral.conversionData.daysToQuote
    ) {
    ...
    if (
      updateStatusDto.status === 'won' &&
      !referral.conversionData.daysToConversion
    ) {
  ...
  async convertToJob(
    ...
    const partner = await this.partnersService.findById(
      referral.partnerId.toString(),
    );
    const commissionAmount = this.partnersService.calculateCommission(
      partner,
      jobValue,
    );
    ...
    await this.partnersService.recordLeadConversion(
      referral.partnerId.toString(),
      jobValue,
    );
  ...
  async markCommissionPaid(
    referralId: string,
    markPaidDto: MarkCommissionPaidDto,
  ): Promise<ReferralDocument> {
    const referral = await this.findById(referralId);

    if (referral.status !== 'won') {
      throw new BadRequestException(
        'Commission can only be paid for won referrals',
      );
    }
    ...
    const paidAmount =
      markPaidDto.paidAmount || referral.commissionDetails.commissionAmount;

    referral.commissionDetails = {
      ...referral.commissionDetails,
      isPaid: true,
      paidDate: markPaidDto.paidDate
    ...
    await this.partnersService.recordCommissionPayment(
      referral.partnerId.toString(),
      paidAmount,
    );
        ...
        $group: {
          ...
          contacted: {
            $sum: {
              $cond: [
                {
                  $in: ['$status', ['contacted', 'qualified', 'quoted', 'won']],
                },
                1,
                0,
              ],
          ...
          qualified: {
            $sum: {
              $cond: [
                { $in: ['$status', ['qualified', 'quoted', 'won']] },
                1,
                0,
              ],
            },

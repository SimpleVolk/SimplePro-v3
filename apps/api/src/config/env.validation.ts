...
const isProductionSecret = (value: string, context: z.RefinementCtx, secretName: string) => {
  ...
  const devPatterns = ['dev', 'test', 'development', 'example', 'demo', 'changeme'];
...
const baseEnvSchema = z.object({
  ...
  REDIS_TLS_ENABLED: z.string().default('false').transform(val => val === 'true'),
  REDIS_CLUSTER_MODE: z.string().default('false').transform(val => val === 'true'),
  ...
  JWT_REFRESH_SECRET: z.string().min(32, 'JWT_REFRESH_SECRET must be at least 32 characters')
    .superRefine((val, ctx) => isStrongSecret(val, ctx, 32))
    .superRefine((val, ctx) => isProductionSecret(val, ctx, 'JWT_REFRESH_SECRET')),
  ...
  STORAGE_PORT: z.string().default('443').transform(val => parseInt(val, 10)),
  STORAGE_ACCESS_KEY: z.string().optional(),
  STORAGE_SECRET_KEY: z.string().optional(),
  STORAGE_USE_SSL: z.string().default('true').transform(val => val === 'true'),
  ...
  EMAIL_PROVIDER: z.enum(['sendgrid', 'ses', 'mailgun', 'postmark', 'other']).default('sendgrid'),
  EMAIL_NOTIFICATIONS_ENABLED: z.string().default('true').transform(val => val === 'true'),
  ...
  SMS_NOTIFICATIONS_ENABLED: z.string().default('false').transform(val => val === 'true'),
  ...
  PUSH_NOTIFICATIONS_ENABLED: z.string().default('false').transform(val => val === 'true'),

  // Rate Limiting
  RATE_LIMIT_GLOBAL: z.string().default('1000').transform(val => parseInt(val, 10)),
  RATE_LIMIT_AUTH: z.string().default('10').transform(val => parseInt(val, 10)),
  RATE_LIMIT_LOGIN: z.string().default('5').transform(val => parseInt(val, 10)),
  RATE_LIMIT_TIER1: z.string().default('20').transform(val => parseInt(val, 10)),
  RATE_LIMIT_TIER2: z.string().default('100').transform(val => parseInt(val, 10)),
  RATE_LIMIT_TIER3: z.string().default('500').transform(val => parseInt(val, 10)),
  RATE_LIMIT_WINDOW_MS: z.string().default('60000').transform(val => parseInt(val, 10)),
  RATE_LIMIT_BLOCK_DURATION_MS: z.string().default('900000').transform(val => parseInt(val, 10)),

  // Logging
  LOG_LEVEL: z.enum(['error', 'warn', 'info', 'debug', 'verbose']).default('info'),
  LOG_FORMAT: z.enum(['json', 'simple']).default('json'),
  LOG_CONSOLE_ENABLED: z.string().default('true').transform(val => val === 'true'),
  LOG_FILE_ENABLED: z.string().default('false').transform(val => val === 'true'),
  LOG_FILE_PATH: z.string().default('/var/log/simplepro/api.log'),
  LOG_FILE_MAX_SIZE: z.string().default('10485760').transform(val => parseInt(val, 10)),
  LOG_FILE_MAX_FILES: z.string().default('10').transform(val => parseInt(val, 10)),
  CLOUDWATCH_LOGS_ENABLED: z.string().default('false').transform(val => val === 'true'),
  CLOUDWATCH_LOG_GROUP: z.string().optional(),
  CLOUDWATCH_LOG_STREAM: z.string().optional(),
  DATADOG_LOGS_ENABLED: z.string().default('false').transform(val => val === 'true'),
  DATADOG_API_KEY: z.string().optional(),

  // Monitoring & Metrics
  PROMETHEUS_ENABLED: z.string().default('true').transform(val => val === 'true'),
  PROMETHEUS_METRICS_PATH: z.string().default('/metrics'),
  PROMETHEUS_PORT: z.string().default('9090').transform(val => parseInt(val, 10)),
  HEALTH_CHECK_ENABLED: z.string().default('true').transform(val => val === 'true'),
  HEALTH_CHECK_PATH: z.string().default('/health'),
  APM_ENABLED: z.string().default('false').transform(val => val === 'true'),
  ...
  SESSION_TTL: z.string().default('604800').transform(val => parseInt(val, 10)),
  ...
  SESSION_COOKIE_HTTP_ONLY: z.string().default('true').transform(val => val === 'true'),
  SESSION_COOKIE_SAME_SITE: z.enum(['strict', 'lax', 'none']).default('lax'),
  SESSION_COOKIE_DOMAIN: z.string().optional(),

  // File Upload
  MAX_FILE_SIZE: z.string().default('52428800').transform(val => parseInt(val, 10)),
  ALLOWED_FILE_TYPES: z.string().default('image/jpeg,image/png,image/gif,application/pdf'),
  MAX_FILES_PER_UPLOAD: z.string().default('10').transform(val => parseInt(val, 10)),
  ...
  HSTS_MAX_AGE: z.string().default('31536000').transform(val => parseInt(val, 10)),
  ...
  PASSWORD_REQUIRE_NUMBER: z.string().default('true').transform(val => val === 'true'),
  PASSWORD_REQUIRE_SPECIAL_CHAR: z.string().default('true').transform(val => val === 'true'),
  ACCOUNT_LOCKOUT_ENABLED: z.string().default('true').transform(val => val === 'true'),
  ACCOUNT_LOCKOUT_ATTEMPTS: z.string().default('5').transform(val => parseInt(val, 10)),
  ACCOUNT_LOCKOUT_DURATION: z.string().default('900').transform(val => parseInt(val, 10)),

  // Feature Flags
  ENABLE_SWAGGER: z.string().default('false').transform(val => val === 'true'),
  ...
  SEED_DATA: z.string().default('false').transform(val => val === 'true'),
  WEBSOCKET_ENABLED: z.string().default('true').transform(val => val === 'true'),
  CREW_AUTO_ASSIGNMENT_ENABLED: z.string().default('true').transform(val => val === 'true'),
  AUTO_JOB_STATUS_ENABLED: z.string().default('true').transform(val => val === 'true'),
  NOTIFICATION_RETRY_ENABLED: z.string().default('true').transform(val => val === 'true'),
  NOTIFICATION_MAX_RETRIES: z.string().default('3').transform(val => parseInt(val, 10)),

  // Webhooks
  WEBHOOK_SECRET: z.string().optional(),
  WEBHOOK_TIMEOUT: z.string().default('5000').transform(val => parseInt(val, 10)),
  ...
  CLUSTER_MODE_ENABLED: z.string().default('false').transform(val => val === 'true'),
  CLUSTER_WORKERS: z.string().default('0').transform(val => parseInt(val, 10)),
  SHUTDOWN_TIMEOUT: z.string().default('30000').transform(val => parseInt(val, 10)),
  DNS_CACHE_TTL: z.string().default('300').transform(val => parseInt(val, 10)),
});
...
const productionEnvSchema = baseEnvSchema.superRefine((data, ctx) => {
  ...
  if (!data.REDIS_TLS_ENABLED && !data.REDIS_HOST.includes('localhost')) {
    ctx.addIssue({
      ...
      message: 'REDIS_TLS_ENABLED should be true for non-localhost connections in production',
    });
  }
...
function logConfigurationSummary(config: any) {
  ...
  logger.log(`  Storage: ${config.STORAGE_PROVIDER} (${config.STORAGE_BUCKET_NAME})`);
  logger.log(`  CORS Origins: ${config.ALLOWED_ORIGINS || 'all (development)'}`);
  logger.log(`  Features:`);
  logger.log(`    - Swagger: ${config.ENABLE_SWAGGER}`);
  logger.log(`    - WebSocket: ${config.WEBSOCKET_ENABLED}`);
  logger.log(`    - Email Notifications: ${config.EMAIL_NOTIFICATIONS_ENABLED}`);

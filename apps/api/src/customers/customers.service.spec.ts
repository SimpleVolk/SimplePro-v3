...
describe('CustomersService', () => {
  ...
  const createMockCustomerModel = () => {
    ...
    mockConstructor.findOne = jest.fn((query) => {
      const email = query?.email?.$regex
        ? query.email.$regex.source.toLowerCase()
    ...
    mockConstructor.countDocuments = jest
      .fn()
      .mockResolvedValue(savedCustomers.size);
  ...
  describe('create', () => {
    ...
    it('should handle case-insensitive email duplication', async () => {
      ...
      await expect(
        service.create(upperCaseEmailDto, 'user123'),
  ...
  describe('update', () => {
    ...
    it('should check for email conflicts during update', async () => {
      ...
      await expect(
        service.update(customerId, updateDto, 'updater123'),
      ).rejects.toThrow(ConflictException);
    });
  ...
  describe('relationship management', () => {
    ...
    it('should update last contact date', async () => {
      ...
      expect(result.lastContactDate!.getTime()).toBeGreaterThanOrEqual(

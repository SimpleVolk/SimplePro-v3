...
import {
  DocumentEntity as DocumentSchema,
  DocumentDocument,
...
export class CustomersService {
  ...
  async findAll(
    ...
    const [total, customers] = await Promise.all([
      ...
      this.customerModel
        .find(query)
        .select(
          'firstName lastName email phone address type status source leadScore tags assignedSalesRep lastContactDate createdAt updatedAt',
    ...
    const data = customers.map((customer) =>
      this.convertCustomerDocument(customer as any),
    );
  ...
  async update(
    ...
    if (
      ...
      const duplicateCustomer = await this.customerModel
        .findOne({
  ...
  async remove(id: string): Promise<void> {
    return this.transactionService.withTransaction(async (session) => {
      ...
      const jobDeleteResult = await this.jobModel
        .deleteMany({ customerId: id }, { session })
        .exec();

      // 3. Delete all customer opportunities
      const opportunityDeleteResult = await this.opportunityModel
        .deleteMany({ customerId: id }, { session })
        .exec();

      // 4. Archive customer documents (don't delete for audit trail)
      const documentArchiveResult = await this.documentModel
        .updateMany(
          { relatedCustomer: id },

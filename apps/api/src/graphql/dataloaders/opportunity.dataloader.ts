...
export class OpportunityDataLoader {
  ...
  private readonly batchOpportunities = new DataLoader<
    string,
    Opportunity | null
  >(async (opportunityIds: readonly string[]) => {
    ...
    opportunities.forEach((opportunity: any) => {
      opportunityMap.set(
        ...
        this.convertOpportunityDocument(opportunity),
      );
    });
  ...
  async loadMany(opportunityIds: string[]): Promise<(Opportunity | null)[]> {
    return this.batchOpportunities.loadMany(opportunityIds) as Promise<
      (Opportunity | null)[]
    >;
  }

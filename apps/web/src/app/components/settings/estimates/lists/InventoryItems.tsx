  ...
  const [items, setItems] = useState<InventoryItem[]>([
    {
      ...
      isFragile: false,
      isHighValue: false,
      packingMaterialsRequired: ['Blankets', 'Shrink Wrap'],
      isActive: true,
    },
    {
      id: '2',
      ...
      isFragile: false,
      isHighValue: true,
      packingMaterialsRequired: ['Appliance Dolly', 'Straps'],
      isActive: true,
    },
    {
      id: '3',
      ...
      isFragile: true,
      isHighValue: true,
      packingMaterialsRequired: ['TV Box', 'Bubble Wrap'],
      isActive: true,
    },
    {
      id: '4',
      ...
      isFragile: false,
      isHighValue: false,
      packingMaterialsRequired: ['Blankets'],
      isActive: true,
    },
    {
      id: '5',
      ...
      isFragile: false,
      isHighValue: true,
      packingMaterialsRequired: ['Appliance Dolly'],
      isActive: true,
    },
    {
      id: '6',
  ...
  const handleSave = () => {
    if (editingItem) {
      setItems((prev) =>
        prev.map((i) =>
          i.id === editingItem.id
            ? { ...(formData as InventoryItem), id: editingItem.id }
            : i,
        ),
      );
    } else {
      setItems((prev) => [
        ...prev,
        { ...(formData as InventoryItem), id: Date.now().toString() },
      ]);
    }
          ...
          <div className={styles.formGrid}>
            ...
            <div className={styles.formGroup}>
              ...
              <input
                ...
                onChange={(e) =>
                  updateFormData('defaultWeight', parseFloat(e.target.value))
                }
                className={styles.input}
                min="0"
                step="0.1"
            ...
            <div className={styles.formGroup}>
              ...
              <input
                type="number"
                value={formData.defaultVolume || 0}
                onChange={(e) =>
                  updateFormData('defaultVolume', parseFloat(e.target.value))
                }
                className={styles.input}
                min="0"
                step="0.1"
            ...
            <div className={styles.checkboxGroup}>
              <label className={styles.checkboxLabel}>
                <input
                  ...
                  onChange={(e) =>
                    updateFormData('requiresSpecialHandling', e.target.checked)
                  }
                />
              ...
              <label className={styles.checkboxLabel}>
                <input
                  ...
                  onChange={(e) =>
                    updateFormData('isFragile', e.target.checked)
                  }
                />
                Fragile Item
              </label>
              <label className={styles.checkboxLabel}>
                <input
                  type="checkbox"
                  checked={formData.isHighValue || false}
                  onChange={(e) =>
                    updateFormData('isHighValue', e.target.checked)
                  }
                />
                High Value Item
              </label>
          ...
          <div className={styles.formSection}>
            ...
            <div className={styles.materialChips}>
              {packingMaterialOptions.map((material) => {
                const isSelected = (
                  formData.packingMaterialsRequired || []
                ).includes(material);
                return (
                  <button
                    key={material}
                    onClick={() => togglePackingMaterial(material)}
                    className={`${styles.materialChip} ${isSelected ? styles.selected : ''}`}
                  >
                    {material}
                  </button>
                );
            ...
            <div key={item.id} className={styles.tableRow}>
              ...
              <div className={styles.flags}>
                {item.requiresSpecialHandling && (
                  <span className={styles.flag}>⚙️ Special</span>
                )}
                {item.isFragile && (
                  <span className={styles.flag}>🔴 Fragile</span>
                )}
                {item.isHighValue && (
                  <span className={styles.flag}>💎 High Value</span>
                )}
              </div>
              <div>
                <span

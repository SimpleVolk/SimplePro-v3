...
describe('DeterministicEstimator', () => {
  ...
  describe('Deterministic Calculations', () => {
    it('should produce identical results for identical inputs', () => {
      const result1 = estimator.calculateEstimate(sampleInputs.studioLocal, 'test-user');
      const result2 = estimator.calculateEstimate(sampleInputs.studioLocal, 'test-user');
      ...
      expect(result1.calculations.appliedRules).toHaveLength(result2.calculations.appliedRules.length);
    });

    it('should produce different hashes for different inputs', () => {
      const result1 = estimator.calculateEstimate(sampleInputs.studioLocal, 'test-user');
      const result2 = estimator.calculateEstimate(sampleInputs.largeLargePiano, 'test-user');
  ...
  describe('Base Price Calculations', () => {
    it('should calculate correct base price for local moves', () => {
      const result = estimator.calculateEstimate(sampleInputs.studioLocal, 'test-user');
    ...
    it('should calculate correct base price for long distance moves', () => {
      const result = estimator.calculateEstimate(sampleInputs.longDistanceHeavy, 'test-user');
    ...
    it('should calculate correct base price for packing only', () => {
      const result = estimator.calculateEstimate(sampleInputs.packingOnly, 'test-user');
    ...
    it('should adjust base price for larger crews', () => {
      const result = estimator.calculateEstimate(sampleInputs.largeLargePiano, 'test-user');
  ...
  describe('Pricing Rules Application', () => {
    it('should apply crew size adjustment for larger crews', () => {
      ...
      const crewRule = result.calculations.appliedRules.find(r => r.ruleId === 'crew_size_adjustment');
    ...
    it('should apply heavy weight surcharge for shipments over 8,000 lbs', () => {
      ...
      const weightRule = result.calculations.appliedRules.find(r => r.ruleId === 'weight_heavy_surcharge');
    ...
    it('should apply piano special handling charge', () => {
      ...
      const pianoRule = result.calculations.appliedRules.find(r => r.ruleId === 'piano_special_handling');
      expect(pianoRule).toBeDefined();
      expect(pianoRule!.priceImpact).toBe(350);
    });

    it('should apply weekend surcharge', () => {
      const result = estimator.calculateEstimate(sampleInputs.weekendChallenge, 'test-user');

      const weekendRule = result.calculations.appliedRules.find(r => r.ruleId === 'weekend_surcharge');
    ...
    it('should apply peak season surcharge', () => {
      ...
      const peakRule = result.calculations.appliedRules.find(r => r.ruleId === 'peak_season_surcharge');
    ...
    it('should apply minimum charge for small local moves', () => {
      const result = estimator.calculateEstimate(sampleInputs.minimalLocal, 'test-user');

      const minRule = result.calculations.appliedRules.find(r => r.ruleId === 'minimum_charge_local');
    ...
    it('should apply fragile items surcharge for large quantities', () => {
      ...
      const fragileRule = result.calculations.appliedRules.find(r => r.ruleId === 'fragile_items_surcharge');
    ...
    it('should apply antique handling charge', () => {
      ...
      const antiqueRule = result.calculations.appliedRules.find(r => r.ruleId === 'antique_handling');
  ...
  describe('Location Handicaps', () => {
    it('should apply stairs handicap at pickup', () => {
      ...
      const stairsHandicap = result.calculations.locationHandicaps.find(h => h.handicapId === 'stairs_pickup');
    ...
    it('should apply long carry handicap', () => {
      ...
      const longCarryHandicap = result.calculations.locationHandicaps.find(h => h.handicapId === 'long_carry_pickup');
    ...
    it('should apply difficult access multiplier', () => {
      ...
      const accessHandicap = result.calculations.locationHandicaps.find(h => h.handicapId === 'difficult_access_pickup');
    ...
    it('should apply parking distance handicap', () => {
      ...
      const parkingHandicap = result.calculations.locationHandicaps.find(h => h.handicapId === 'parking_distance_pickup');
    ...
    it('should apply narrow hallways multiplier', () => {
      ...
      const hallwaysHandicap = result.calculations.locationHandicaps.find(h => h.handicapId === 'narrow_hallways_pickup');
  ...
  describe('Price Breakdown', () => {
    it('should provide detailed price breakdown', () => {
      ...
      expect(result.calculations.breakdown.seasonalAdjustment).toBeGreaterThan(0);
    ...
    it('should calculate totals correctly', () => {
      ...
      const breakdown = result.calculations.breakdown;
      const calculatedSubtotal = breakdown.baseLabor + breakdown.materials +
                                breakdown.transportation + breakdown.locationHandicaps +
                                breakdown.specialServices + breakdown.seasonalAdjustment;

      expect(Math.abs(breakdown.subtotal - calculatedSubtotal)).toBeLessThan(0.01); // Allow for rounding
  ...
  describe('Edge Cases', () => {
    it('should handle zero special items gracefully', () => {
      ...
      expect(result.calculations.appliedRules.some(r => r.ruleId.includes('fragile'))).toBe(false);
    });
  ...
  describe('Rule Priority and Order', () => {
    it('should apply rules in correct priority order', () => {
      ...
      const baseRules = result.calculations.appliedRules.filter(r =>
        r.ruleId.includes('base') || r.ruleId.includes('crew')
      );
      const specialRules = result.calculations.appliedRules.filter(r =>
        r.ruleId.includes('piano') || r.ruleId.includes('antique')
      );
    ...
    it('should not apply inactive rules', () => {
      ...
      const result = testEstimator.calculateEstimate(sampleInputs.largeLargePiano, 'test-user');
    ...
    it('should respect service type restrictions', () => {
      ...
      const longDistanceRules = result.calculations.appliedRules.filter(r =>
        r.ruleId.includes('distance') && r.ruleId !== 'parking_distance'
      );

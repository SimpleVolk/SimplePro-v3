...
const hasEnvVars = dockerCompose.includes('${MONGODB_PASSWORD}') &&
...
test(
  ...
  !hasHardcodedMongo,
  hasHardcodedMongo ? 'Found "simplepro_dev_2024" in docker-compose.dev.yml' : 'Uses ${MONGODB_PASSWORD} variable'
);

test(
  'No hardcoded Redis password',
  !hasHardcodedRedis,
  hasHardcodedRedis ? 'Found "simplepro_redis_2024" in docker-compose.dev.yml' : 'Uses ${REDIS_PASSWORD} variable'
);

test(
  'No hardcoded MinIO password',
  !hasHardcodedMinio,
  hasHardcodedMinio ? 'Found "simplepro_minio_2024" in docker-compose.dev.yml' : 'Uses ${MINIO_ROOT_PASSWORD} variable'
);
...
console.log('-'.repeat(80));
const partnerJwtStrategy = fs.readFileSync('apps/api/src/auth/strategies/partner-jwt.strategy.ts', 'utf8');
...
const hasErrorMessage = partnerJwtStrategy.includes('JWT_SECRET configuration failed');

test(
  'No weak default secret fallback',
  !hasWeakFallback,
  hasWeakFallback ? 'Found "default-secret-key" in partner-jwt.strategy.ts' : 'Removed weak fallback'
);
...
console.log('-'.repeat(80));
const documentsController = fs.readFileSync('apps/api/src/documents/documents.controller.ts', 'utf8');
const documentsService = fs.readFileSync('apps/api/src/documents/documents.service.ts', 'utf8');

const usesPostMethod = documentsController.includes('@Post(\'shared/:token/access\')') &&
                       documentsController.includes('@Post(\'shared/:token/download\')');
const hasRateLimit = documentsController.includes('@Throttle({ default: { limit: 5, ttl: 3600000 } })');
const usesBodyDto = documentsController.includes('AccessSharedDocumentDto');
const hasAuditLogging = documentsService.includes('SECURITY AUDIT') &&
...
console.log('-'.repeat(80));
const websocketGateway = fs.readFileSync('apps/api/src/websocket/websocket.gateway.ts', 'utf8');
...
const hasRateLimitCheck = websocketGateway.includes('checkEventRateLimit(client.id)');
...
test(
  ...
  docContent.includes('### 1.') && docContent.includes('### 2.') &&
  docContent.includes('### 3.') && docContent.includes('### 4.'),
...
if (failed === 0) {
  ...
  console.log('  3. ✓ Document sharing endpoint secured with POST + rate limiting');
  console.log('  4. ✓ WebSocket connection limits enforced with auth-first approach');
  console.log('');
  console.log('Next steps:');
  console.log('  1. Create .env.docker file with secure passwords');
  console.log('  2. Update frontend to use new POST endpoints for document sharing');
